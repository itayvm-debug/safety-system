/**
 * Playwright fixture: authenticated page for subcontractors QA tests.
 *
 * Safety guarantees:
 *  - Loads stored auth state (Internal QA) from global-setup.
 *  - Navigates to /subcontractors before each test and verifies the active
...
async function verifySafetyGates(page: Page, companyId: string) {
  ...
  if (!html.includes(QA_COMPANY)) {
    throw new Error(
      ...
      'Refusing to run subcontractors tests against unknown tenant.',
    );
  }
