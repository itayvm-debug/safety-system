/**
 * Playwright fixture: authenticated page for heavy equipment QA tests.
 *
 * Safety guarantees:
 *  - Loads stored auth state (Internal QA) from global-setup.
 *  - Navigates to /heavy-equipment before each test and verifies the active
...
async function verifySafetyGates(page: Page, companyId: string) {
  ...
  if (!html.includes(QA_COMPANY)) {
    throw new Error(
      ...
      'Refusing to run heavy-equipment tests against unknown tenant.',
    );
  }
