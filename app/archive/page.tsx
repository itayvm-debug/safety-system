...
export default async function ArchivePage() {
  ...
  const [workers, vehicles, heavy, lifting, subcontractors] = await Promise.all([
    ...
    supabase.from('vehicles').select('id, vehicle_number, vehicle_type, archived_at, archived_by').eq('company_id', companyId).eq('is_archived', true).order('archived_at', { ascending: false }),
    ...
    supabase.from('subcontractors').select('id, name, archived_at, archived_by').eq('company_id', companyId).eq('is_archived', true).order('archived_at', { ascending: false }),
  ]);
