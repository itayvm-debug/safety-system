...
import { LiftingEquipment, Subcontractor } from '@/types';

interface Props { equipment?: LiftingEquipment; }

export default function LiftingForm({ equipment }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const url = isEdit ? `/api/lifting-equipment/${equipment!.id}` : '/api/lifting-equipment';
      ...
      router.push(`/lifting-equipment/${data.id}`);
      router.refresh();
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <select value={form.subcontractor_id} onChange={(e) => setForm({ ...form, subcontractor_id: e.target.value })}
